...
int mididrv_resources_init(void)
{
   // return resources_register_string(resources_string);
	...
	set_midi_name("CX16MIDI", nil);
	set_midi_in_name("CX16MIDI_IN", nil);
	set_midi_out_name("CX16MIDI_OUT", nil);
...
static int create_client(void)
{
    if (midi_client_usage == 0) {
        printf( "Opening MIDI client '%s'", midi_name);
...
static void dispose_client(void)
{
    midi_client_usage--;
    if (midi_client_usage == 0) {
        printf( "Closing MIDI client");
        OSStatus status = MIDIClientDispose(midi_client);
        if (status != noErr) {
           printf(  "Error disposing client!");
        }
    }
}
...
int mididrv_in_open(void)
{
    ...
    if (midi_destination_usage == 0) {
        printf( "Opening MIDI-In port '%s'", midi_in_name);
        ...
        CFRelease(name);
        if (status != noErr) {
            printf( "Error creating MIDI-In port!");
            return -1;
        }
    }
...
int mididrv_out_open(void)
{
    ...
    if (midi_source_usage == 0) {
        printf( "Opening MIDI-Out port '%s'", midi_out_name);
        ...
        CFRelease(name);
        if (status != noErr) {
            printf(  "Error creating MIDI-Out port!");
            return -1;
        }
    }
    midi_source_usage ++;
...
void mididrv_in_close(void)
{
    midi_destination_usage --;
    if (midi_destination_usage == 0) {
        printf(  "Closing MIDI-In port");

        OSStatus status = MIDIEndpointDispose(midi_destination);
        if (status != noErr) {
            printf(  "Error disposing MIDI-In port!");
        }
    }
...
void mididrv_out_close(void)
{
    midi_source_usage --;
    if (midi_source_usage == 0) {
        printf( "Closing MIDI-Out port");

        OSStatus status = MIDIEndpointDispose(midi_source);
        if (status != noErr) {
            printf(  "Error disposing MIDI-Out port!");
        }
    }
...
void mididrv_out(uint8_t b)
{
    ...
    if (out_index >= thres) {
        ...
        if (status != noErr) {
            printf( "Failed to output data on MIDI-Out device.");
        }
    }
}
