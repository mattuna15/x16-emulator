...
midi_interface_t midi_interface[] = {
    ...
    { "Sequential", 0xde00, 0, 2, 1, 3, 0xff, 1, 1, MIDI_MODE_SEQUENTIAL },
    /* Passport & Syntech */
    { "Passport", 0xde00, 8, 8, 9, 9, 0xff, 1, 1, MIDI_MODE_PASSPORT },
    /* DATEL/Siel/JMS */
    { "DATEL", 0xde00, 4, 6, 5, 7, 0xff, 2, 1, MIDI_MODE_PASSPORT },
    /* Namesoft */
    { "Namesoft", 0xde00, 0, 2, 1, 3, 0xff, 1, 2, MIDI_MODE_NAMESOFT },
    /* Electronics - Maplin magazine */
    { "Maplin", 0xdf00, 0, 0, 1, 1, 0xff, 2, 0, MIDI_MODE_MAPLIN },
...
static uint8_t midistatus;     /* status register */
...
{
    return (int)((MHZ * 1000000 * 9) / 31250);
}
...
//    }
	midi_set_mode(MIDI_MODE_SEQUENTIAL, 0);
    mididrv_init();
	midi_resources_init();
	midi_activate(0);
...
static void midi_suspend(void)
{
    ...
    printf( "suspend");
#endif
    midistatus = MIDI_STATUS_DEFAULT;
    intx = 0;
...
#ifdef DEBUG
        printf( "store tx: %02x", b);
#endif
        if ((midistatus & MIDI_STATUS_TDRE) && !(MIDI_CTRL_CD(ctrl) == MIDI_CTRL_RESET)) {
            midistatus &= ~MIDI_STATUS_TDRE;
            txdata = b;
            tx_irq = 0;
            if (!intx) {
//                midi_alarm_clk = maincpu_clk + 1;
...
void midi_get_byte(uint8_t *b) {
	...
	midi_data_status = MIDI_DATA_BUSY;
	midi_data_status =  mididrv_in(b);
...
void midi_put_byte(uint8_t b) {
	...
	mididrv_out(b);
	midi_ticks = get_midi_ticks();
	midi_data_status = MIDI_DATA_STATUS_EMPTY;
}
...
#ifdef DEBUG
        printf( "read status: %02x", midistatus);
#endif
        midi_last_read = status;
    } else if (a == midi_interface[midi_mode].rx_addr) {
#ifdef DEBUG
        printf( "read rx: %02x (%02x)", rxdata, midistatus);
#endif
        midistatus &= ~MIDI_STATUS_OVRN;
        if (rx_irq) {
            midistatus &= ~MIDI_STATUS_IRQ;
            rx_irq = 0;
            midi_update_int();
        }
        if (midistatus & MIDI_STATUS_RDRF) {
            midistatus &= ~MIDI_STATUS_RDRF;
...
static void int_midi(CLOCK offset, void *data)
{
...
#if 0 /*def DEBUG */
    printf( "int_midi(offset=%ld, clk=%d", (long int)offset, (int)maincpu_clk);
#endif
    if ((intx == 2) && (fd_out >= 0)) {
        mididrv_out(txdata);
    }
    ...
    if ((fd_in >= 0) && (!(midistatus & MIDI_STATUS_RDRF)) && (mididrv_in(&rxdata) == 1)) {
        midistatus |= MIDI_STATUS_RDRF;
        rxirq = 1;
#ifdef DEBUG
        printf( "int got %02x", rxdata);
#endif
    }
    ...
    if (rxirq && (ctrl & MIDI_CTRL_RIE)) {
        rx_irq = 1;
#ifdef DEBUG
        printf( "int_midi rx IRQ offset=%ld, clk=%d", (long int)offset, (int)clockticks6502);
#endif
    }
    ...
    if (MIDI_CTRL_IS_TX_IRQ(ctrl)) {
        tx_irq = 1;
#ifdef DEBUG
        printf( "int_midi tx IRQ offset=%ld, clk=%d", (long int)offset, (int)clockticks6502);
#endif
    }
